...
const PostDetail: React.FC<PostDetailProps> = ({ isLoggedIn }) => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await axios.get(`https://localhost:443/post_data/get-posts/${id}`);
    ...
    const fetchComments = async () => {
      try {
        const response = await axios.get(`https://localhost:443/post_data/get-posts/${id}/comments`);
    ...
    const fetchCurrentUser = async () => {
      ...
      if (token) {
        try {
          const userResponse = await axios.get('https://localhost:443/user_data/auth/user', {
  ...
  const handleDeletePost = async () => {
    if (window.confirm('정말로 이 게시글을 삭제하시겠습니까?')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`https://localhost:443/post_data/posts/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        alert('게시글이 삭제되었습니다.');
  ...
  const handleLike = async () => {
    ...
    try {
      await axios.post(`https://localhost:443/post_data/posts/${id}/like`,{},{headers:{'Authorization': `Bearer ${token}`,}});
  ...
  const handleDislike = async () => {
    ...
    try {
      await axios.post(`https://localhost:443/post_data/posts/${id}/dislike`,{},{headers:{'Authorization': `Bearer ${token}`,}});
  ...
  const handleDeleteComment = async (commentId: number) => {
    if (window.confirm('정말로 이 댓글을 삭제하시겠습니까?')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`https://localhost:443/post_data/posts/${id}/comments/${commentId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleSaveEditComment = async (commentId: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `https://localhost:443/post_data/posts/${id}/comments/${commentId}`,
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      ...
      await axios.post(
        `https://localhost:443/post_data/posts/${id}/comments`,
        { content: newComment },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setNewComment('');
      const updatedComments = await axios.get(`https://localhost:443/post_data/posts/${id}/comments`);
